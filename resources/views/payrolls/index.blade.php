...
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto">المرتبات</h4><span class="text-muted mt-1 tx-13 mr-2 mb-0">/ صرف
                المرتبات</span>
        </div>
    </div>
</div>
...
@if (session()->has('add_Payroll'))
        <script>
            window.onload = function() {
                notif({
                    msg: "تم صرف المرتبات بنجاح",
                    type: "success"
                })
            }
        </script>
        ...
        <div class="card">
            <div class="card-header pb-0">
                <div class="col-sm-1 col-md-2">
                    <form class="parsley-style-1" id="selectForm2" autocomplete="off" name="selectForm2" action="{{route('payrolls.store')}}" method="post">
                        {{csrf_field()}}
                      <button class="btn btn-sm btn-success">صرف</button>
                    </form>
                </div>
            ...
            <div class="card-body" id="print">
                <div class="table-responsive hoverable-table">
                  <center>  <h2> <span> قائمة اعضاء هيئة التدريس لصرف المرتبات</span> </h2> </center>
                    <table class="table table-hover"  style=" text-align: center;">
                        <thead>
                            <tr>
                                ...
                                <th class="wd-15p border-bottom-0">اجمالي المرتب</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($salaries as $key => $salary)
                                <tr> 
                                    ...
                                    <td>{{ number_format($salary->amount )}} جنية</td>
                                    <td>{{ number_format($salary->bonuses->sum('amount')) }} جنية</td> 
                                    <td>{{ number_format($salary->bonuses->sum('amount') + $salary->amount) }} </td> 
